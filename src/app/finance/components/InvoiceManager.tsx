...
export default function InvoiceManager({ dateFrom, dateTo }: { dateFrom: string; dateTo: string }) {
  ...
  return (
    <div className="space-y-6">
      {/* ── Actions Bar ─────────────────────────────────────────────────── */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900/50 p-4 rounded-2xl border border-slate-800 shadow-lg">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-blue-600/20 flex items-center justify-center text-blue-400 text-xl border border-blue-500/20">
            🥩
          </div>
          <div>
            <h2 className="text-white font-bold">Nguyên Vật Liệu</h2>
            <p className="text-xs text-slate-500">Quản lý hóa đơn nhập hàng & OCR</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <label htmlFor="invoice-upload"
            className="cursor-pointer flex items-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white px-5 py-2.5 rounded-xl text-sm font-bold transition-all active:scale-95 shadow-lg shadow-emerald-900/40">
          ...
          <button
            ...
            className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-slate-200 px-4 py-2.5 rounded-xl text-sm font-bold transition-all border border-slate-700 shadow-lg"
          >
            ✍️ Nhập tay
          </button>
          ...
          <button
            onClick={exportCSV}
            className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-slate-400 px-4 py-2.5 rounded-xl text-sm font-bold transition-all border border-slate-700"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex flex-col gap-3">
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input
                type="text"
                placeholder="Tìm nhà cung cấp, số hóa đơn..."
                value={search}
